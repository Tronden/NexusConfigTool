...
class ExcelCreationToolGUI:
    ...
    def show_gen_settings(self, event=None):
        ...
        for i in range(num_generators):
            ...
            settings_frame.grid(row=i, column=0, sticky="ew", padx=5, pady=5, columnspan=2)
    ...
    def on_com_type_selected(self, event, index):
        ...
        settings_frame = self.gen_settings[index]["settings_frame"]
        if com_type == "RTU":
            self.gen_settings[index]["settings"] = self.show_rtu_settings(settings_frame)
        elif com_type == "TCP":
            self.gen_settings[index]["settings"] = self.show_tcp_settings(settings_frame)
    ...
    def show_rtu_settings(self, settings_frame):
        ...
        return self.rtu_settings_widgets
    ...
    def show_tcp_settings(self, settings_frame):
        ...
        self.tcp_settings_widgets = {}

        def on_entry_change(event, label_text, entry_widget):
            self.tcp_settings_widgets[label_text] = entry_widget.get()
        ...
        self.tcp_settings_widgets["IP Address"] = self.ip_address_entry.get()
        self.ip_address_entry.bind("<KeyRelease>", lambda event, label_text="IP Address", entry_widget=self.ip_address_entry: on_entry_change(event, label_text, entry_widget))
        ...
        self.tcp_settings_widgets["Port"] = self.port_entry.get()
        self.port_entry.bind("<KeyRelease>", lambda event, label_text="Port", entry_widget=self.port_entry: on_entry_change(event, label_text, entry_widget))
        ...
        self.tcp_settings_widgets["Timeout"] = self.timeout_entry.get()
        self.timeout_entry.bind("<KeyRelease>", lambda event, label_text="Timeout", entry_widget=self.timeout_entry: on_entry_change(event, label_text, entry_widget))
        
        return self.tcp_settings_widgets
    ...
    def create_file(self):
        ...
        num_generators = self.num_generators_combobox.get()
        rtumodifications = {}
        tcpmodifications = {}
